...
const Profile = ({ id }: { id: number }) => {
	...
	useEffect(() => {
		...
		// fetch(`/api/lists?&seller=${user.data.address}`)
		// // fetch(`/api/getLists`)
		// 	.then((res) => res.json())
		// 	.then((data) => {
		// 		setLists(data.data);
	...
	const buyLists  = lists["data"]?lists["data"].filter((l) => l.type === 'BuyList'):[];
	const sellLists = lists["data"]?lists["data"].filter((l) => l.type === 'SellList'):[];
					...
					<div className="w-full md:w-5/6 m-auto">
						{sellLists.length > 0 && (
							<div className="mb-4">
								<Label title="Buy Ads" />
								<ListsTable
									lists={lists["data"].filter((l) => l.type === 'SellList')}
						...
						{buyLists.length > 0 && (
							<div>
								...
								<ListsTable
									lists={lists["data"].filter((l) => l.type === 'BuyList')}
									hideLowAmounts={!isLoggedUser}
								/>
							</div>
						)}
