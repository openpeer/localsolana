...
const useShyft = () => {
  ...
  try {
    ...
    if (parsedError.InstructionError && Array.isArray(parsedError.InstructionError)) {
      ...
      if (instructionError.Custom !== undefined) {
        console.log('This is an InstructionError with a custom error code:', instructionError.Custom);
        // You can now handle the custom error code, e.g.
        //handleCustomError(instructionError.Custom);
        idl.errors[instructionError.custom];
      } else {
        console.log('This is an InstructionError, but no custom error was found');
      }
    } else {
  ...
  const getAccountInfo = async (address: string) => {
    ...
    const accountInfo = await shyft?.connection.getAccountInfo(new PublicKey(address));
